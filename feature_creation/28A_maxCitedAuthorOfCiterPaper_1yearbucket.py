...
def calculateAuthorCitationCount(a,authorPublications,paperYear,year,paperCiter):
...
with open("paperCiterFinal","rb") as pc:
	for line in pc:
		...
		for i in range(sz):
			try:
				if paperYear[element[i]]<=year+1:
					if paperAuthor.has_key(element[i]):
						m=0
						for a in paperAuthor[element[i]]:
							a_pc=calculateAuthorCitationCount(a,authorPublications,paperYear,year,paperCiter)
							# print a,a_pc
							if a_pc>m:
								m=a_pc
								key=a
						paperAuthorCitationCount[paper][key]=m
...
############ dump to json ################################
with open('28A_maxpaperAuthorCitationCount_1yearbucket.json', 'w') as fp5:
