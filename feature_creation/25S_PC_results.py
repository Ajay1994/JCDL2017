...
avgDist={}
with open('pcAvgD.txt','r') as f:
	for line in f:
		entry=line[:-1].split("\t")
		avgDist[entry[0]]=entry[1]

op=open("2_OCT_PC_results_dist3.txt","a")
	...
	for line in in_file:
		try:
			...
			if paperCitationCount.has_key(entry[0]) and avgDist.has_key(entry[0]) and float(avgDist[entry[0]])>=2:# and float(avgDist[entry[0]])<2:
				...
				# print entry[1]
				if float(entry[1])>=float(30):# and float(entry[1])<float(30):
			...
			continue
				# 
# op.write("Correlation Values for pc(taken mean of max publication count citer author) and citaion count after 10 years\n\n0-2:\n\n")
op.write("\n\npc>=30\n\n")
op.write("(0-5)\t")
op.write(str(len(max_p1)))
op.write("\t")
op.write(str((corr(max_p1,list_t10_1))))
op.write("\n")

# op.write("(5-10)\t")
# op.write(str(len(max_p2)))
# op.write("\t")
# op.write(str((corr(max_p2,list_t10_2))))
# op.write("\n")

# op.write("(10-20)\t")
# op.write(str(len(max_p3)))
# op.write("\t")
# op.write(str((corr(max_p3,list_t10_3))))
# op.write("\n")

# op.write("(20-100)\t")
# op.write(str(len(max_p4)))
# op.write("\t")
# op.write(str((corr(max_p4,list_t10_4))))
# op.write("\n")

op.write("(>=5)\t")
op.write(str(len(max_p6)))
op.write("\t")
op.write(str((corr(max_p6,list_t10_6))))
op.write("\n")

op.write("All\t")
op.write(str(len(max_p5)))
op.write("\t")
op.write(str((corr(max_p5,list_t10_5))))
op.write("\n")
